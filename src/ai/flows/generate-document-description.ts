...
let cachedGenerateDocumentDescriptionFlow:
  | ((input: GenerateDocumentDescriptionInput) => Promise<GenerateDocumentDescriptionOutput>)
  | null = null;

function getGenerateDocumentDescriptionFlow() {
  if (cachedGenerateDocumentDescriptionFlow) {
    return cachedGenerateDocumentDescriptionFlow;
  }
...
Provide only the description, without any conversational filler or extra formatting. The description should be suitable for display in a list of documents.`,
  ...
  cachedGenerateDocumentDescriptionFlow = ai.defineFlow(
    {
    ...
    async (input) => {
      ...
      if (!output) {
        throw new Error('Genkit returned no document description output.');
      }
...
export async function generateDocumentDescription(
  ...
  return getGenerateDocumentDescriptionFlow()(input);
}
