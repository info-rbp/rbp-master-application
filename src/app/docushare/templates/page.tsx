...
export default async function DocuShareCategoryPage() {
  ...
  const suites = allSuites.filter((suite) => suite.contentType === 'templates');
    ...
    <div>
      ...
      <section className="py-16 md:py-24"><div className="container mx-auto px-4 md:px-6">
        {suites.length === 0 ? <Alert><AlertTitle>No published content available</AlertTitle><AlertDescription>Content will appear here when published.</AlertDescription></Alert> : (
          <div className="grid gap-6 md:grid-cols-2">{suites.map((suite) => <Card key={suite.id}><CardHeader><CardTitle><Link className="hover:underline" href={`/docushare/templates/${suite.slug ?? suite.id}`}>{suite.name}</Link></CardTitle></CardHeader><CardContent><p className="text-sm text-muted-foreground">{suite.summary ?? suite.description}</p><p className="text-xs text-muted-foreground mt-2">Resources: {suite.documents.length}</p></CardContent></Card>)}</div>
        )}
      </div></section>
    </div>
  );
