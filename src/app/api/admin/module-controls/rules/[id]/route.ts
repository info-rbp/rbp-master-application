...
export async function PUT(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const updated = await getService().updateModuleRule(id, { ...body.data, updatedBy: context.session.user.id, expectedVersion: body.data?.expectedVersion });
    await getAudit().record({ eventType: 'module.control.updated', action: 'update', category: 'configuration', tenantId: context.session.activeTenant.id, workspaceId: context.session.activeWorkspace?.id, actorType: 'user', actorId: context.session.user.id, actorDisplay: context.session.user.displayName, subjectEntityType: 'module_control', subjectEntityId: id, sourceSystem: 'platform', correlationId, outcome: 'success', severity: 'warning', metadata: { moduleKey: updated.moduleKey, enabled: updated.enabled, visible: updated.visible }, sensitivity: 'internal' });
...
export async function DELETE(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const updated = await getService().disableModuleRule(id, { updatedBy: context.session.user.id, expectedVersion: expectedVersion ? Number(expectedVersion) : undefined });
    await getAudit().record({ eventType: 'module.control.disabled', action: 'disable', category: 'configuration', tenantId: context.session.activeTenant.id, workspaceId: context.session.activeWorkspace?.id, actorType: 'user', actorId: context.session.user.id, actorDisplay: context.session.user.displayName, subjectEntityType: 'module_control', subjectEntityId: id, sourceSystem: 'platform', correlationId, outcome: 'success', severity: 'warning', metadata: { moduleKey: updated.moduleKey, enabled: updated.enabled, visible: updated.visible }, sensitivity: 'internal' });
    return ok(updated, correlationId);
  } catch (error) { return fail(toFeatureControlApiError(error), correlationId); }
}
