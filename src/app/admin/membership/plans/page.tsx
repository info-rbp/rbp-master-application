...
export default function AdminMembershipPlansPage() {
  ...
  const isSquareMappingMissing = (plan: Pick<MembershipPlan, 'active' | 'squareSubscriptionPlanVariationId'>) => plan.active && !plan.squareSubscriptionPlanVariationId;
  ...
  const handleCreate = async (event: FormEvent<HTMLFormElement>) => {
    ...
    if (form.active && !form.squareSubscriptionPlanVariationId.trim()) {
      toast({ variant: 'destructive', title: 'Square mapping required', description: 'Active plans must include a Square plan variation ID.' });
      return;
    }
      ...
      <Card>
        ...
        <CardContent>
          <form className="grid gap-4 md:grid-cols-2" onSubmit={handleCreate}>
            ...
            <div className="space-y-2"><Label>Interval</Label><Input value={form.interval} onChange={(e) => setForm({ ...form, interval: e.target.value })} required /></div>
            ...
            <div className="flex items-center gap-2 md:col-span-2"><input type="checkbox" checked={form.active} onChange={(e) => setForm({ ...form, active: e.target.checked })} /><Label>Active</Label></div>
      ...
      <Card>
        ...
        <CardContent className="space-y-3">
          {loading ? <p className="text-muted-foreground">Loading plans...</p> : plans.length === 0 ? <p className="text-muted-foreground">No plans found.</p> : plans.map((plan, index) => (
            <div key={plan.id} className="grid gap-2 rounded border p-3 md:grid-cols-10">
              <Input value={plan.name} onChange={(e) => setPlans((prev) => prev.map((p, i) => i === index ? { ...p, name: e.target.value } : p))} />
              ...
              <Input value={plan.interval} onChange={(e) => setPlans((prev) => prev.map((p, i) => i === index ? { ...p, interval: e.target.value } : p))} />
              ...
              {isSquareMappingMissing(plan) ? <p className="text-xs text-destructive md:col-span-10">This plan is active but cannot be sold until Square Plan Variation ID is set.</p> : null}
            </div>
          ))}
        </CardContent>
      </Card>
