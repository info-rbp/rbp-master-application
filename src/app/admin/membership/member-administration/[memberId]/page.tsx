  ...
  return (
    <div className="space-y-6 p-4 md:p-8">
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        ...
        <Card><CardHeader><CardTitle>Membership summary</CardTitle></CardHeader><CardContent className="space-y-1 text-sm"><p>Tier: {member.membershipTier}</p><p>Status: {member.membershipStatus}</p><p>Access expiry: {member.accessExpiry ? new Date(member.accessExpiry).toLocaleString() : '—'}</p><p>Plan reference: {member.subscriptionPlanId || '—'}</p><p>Square subscription: {member.squareSubscriptionId || '—'}</p><p>Square customer: {member.squareCustomerId || '—'}</p><p>Last payment status: {member.lastPaymentStatus || '—'}</p><p>Last payment date: {member.lastPaymentAt ? new Date(member.lastPaymentAt).toLocaleString() : '—'}</p><p>Override: {member.overrideEnabled ? 'Enabled' : 'Not enabled'}</p></CardContent></Card>
      </div>
