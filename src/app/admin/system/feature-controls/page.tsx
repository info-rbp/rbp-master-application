...
export default async function FeatureControlsPage() {
  ...
  return <FeatureControlsClient catalog={catalog} assignments={assignments} moduleRules={moduleRules} evaluations={evaluations} modules={modules} canManage={canPermission(response.session.effectivePermissions, 'feature_flags', 'manage')} canManageModules={canPermission(response.session.effectivePermissions, 'module_controls', 'manage')} />;
}
