...
export async function savePartnerOffer(input: Partial<PartnerOffer> & Pick<PartnerOffer, 'title' | 'description' | 'link'>) {
  ...
  const saved = input.id
    ...
    : await createPartnerOffer({
        ...
        partnerOverview: input.partnerOverview,
        partnerServices: input.partnerServices,
...
export async function saveKnowledgeArticle(input: Partial<KnowledgeArticle> & Pick<KnowledgeArticle, 'title' | 'slug' | 'content'>) {
  ...
  const saved = input.id
    ...
    : await createKnowledgeArticle({
        ...
        guideSections: input.guideSections,
        downloadableResources: input.downloadableResources,
        downloadableResourcesSummary: input.downloadableResourcesSummary,
        toolComponents: input.toolComponents,
        howItWorks: input.howItWorks,
        howItWorksSteps: input.howItWorksSteps,
        whenToUse: input.whenToUse,
        exampleApplication: input.exampleApplication,
        exampleSteps: input.exampleSteps,
        embeddedResources: input.embeddedResources,
        embeddedResourcesSummary: input.embeddedResourcesSummary,
        relatedResources: input.relatedResources,
        relatedResourcesSummary: input.relatedResourcesSummary,
